 ...
 */
 /*100ask_frank此函数在此文件没有被使用*/
static int __maybe_unused omap_nand_switch_ecc(uint32_t hardware, uint32_t eccstrength)
{
	...
	printf("100ask_frank---hardware=%d\n",hardware);/*100ask_frank*/
	printf("100ask_frank---eccstrength=%d\n",eccstrength);/*100ask_frank*/	
	if (hardware) {
		if (eccstrength == 1) {
...
int board_nand_init(struct nand_chip *nand)
{
	...
	printf("100ask_frank_nand  drivers/mtd/nand/omap_gpmc.c---board_nand_init\n");/*100ask_frank*/
...
#ifndef CONFIG_SPL_BUILD
 /*100ask_frank,这里如果不加入#ifndef CONFIG_SPL_BUILD，
  *否则为SPL的代码，如nand_bbt中的函数找不到，而编译失败
  *加入#ifndef CONFIG_SPL_BUILD,则为u-boot.img中的代码
 */
//extern void omap_select_ecc_scheme(nand_ecc_modes_t hardware, int32_t mode);
 ...
 */
 /*100ask_frank此函数在此文件没有被使用*/
static int  _omap_nand_switch_ecc(uint32_t hardware, uint32_t eccstrength)
{
	struct nand_chip *nand;
	struct mtd_info *mtd;
	int err = 0;
...
static int do_switch_ecc(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (strncmp(argv[1], "hw", 2) == 0) {
		...
		printf("type=%d\n",type);
		_omap_nand_switch_ecc(NAND_ECC_HW, type);
	}
	else if (strncmp(argv[1], "sw", 2) == 0)
		_omap_nand_switch_ecc(NAND_ECC_SOFT, 0);
	else
		goto usage;
